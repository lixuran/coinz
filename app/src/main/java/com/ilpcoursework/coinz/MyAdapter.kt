        ...
        holder.bankingButton.setOnClickListener { view ->
            ...
            {}
                ...
                when (myDataset[position].currency) {
                    ...
                    userstore.myshils-=myDataset[position].value}
                    "DOLR" -> {rate = userstore.dolrrate
                        userstore.mydolrs-=myDataset[position].value}
                    "QUID" -> {rate = userstore.quidrate
                        userstore.myquids-=myDataset[position].value}

                    "PENY" ->{ rate = userstore.penyrate
                        userstore.mypenys-=myDataset[position].value}

                }
                val addition = myDataset[position].value * rate
                userstore.gold = userstore.gold + addition
                userstore.coins.remove(myDataset[position])

                helperfunctions.updateUser(userstore,"myadapter")
                activity2.updateuser(userstore,userstore.coins.indexOf(myDataset[position]))
            }
        }
    }
